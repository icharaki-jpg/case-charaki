...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (!expert[0]) return NextResponse.json({ error: "پرونده پیدا نشد." }, { status: 404 });
    const result = await db.select().from(cases)
      .where(and(eq(cases.id, id), eq(cases.expertId, expert[0].id))).limit(1);
    if (!result[0]) return NextResponse.json({ error: "پرونده پیدا نشد." }, { status: 404 });
    return NextResponse.json({ case: { ...result[0], createdAt: result[0].createdAt.toISOString() } });
  } catch (error) {
    console.error("case GET failed", error);
    return NextResponse.json({ error: "دریافت پرونده انجام نشد." }, { status: 500 });
  }
}
